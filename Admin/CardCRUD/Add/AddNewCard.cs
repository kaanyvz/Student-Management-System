...
namespace schoolManagementSystem.Admin.CardCRUD.Add
{
    public partial class AddNewCard : Form
    {
        ...
        public AddNewCard(string adminUsername, string schoolName)
        {
        ...
        private void AddCardButtonColumn()
        {
            // Add a button column to the DataGridView
            DataGridViewButtonColumn buttonColumn = new DataGridViewButtonColumn();
            buttonColumn.Name = "addCardButtonColumn";
            buttonColumn.Text = "Add Card";
            buttonColumn.HeaderText = "Add Card";
        ...
        {
            if (e.ColumnIndex == this._dataGridView.Columns["addCardButtonColumn"].Index && e.RowIndex >= 0)
            {
        ...
        private void _dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == this._dataGridView.Columns["addCardButtonColumn"].Index && e.RowIndex >= 0)
            {
                ...
                AddNewCardInformations addNewCardInformations = new AddNewCardInformations(adminUsername, schoolName, studentId);
                addNewCardInformations.StartPosition = FormStartPosition.Manual;
                addNewCardInformations.Location = this.Location;
                this.Hide();
                addNewCardInformations.ShowDialog();
                this.Close();
        ...
        {
            if (e.RowIndex >= 0 && e.ColumnIndex == this._dataGridView.Columns["addCardButtonColumn"].Index)
            {
                e.Paint(e.CellBounds, DataGridViewPaintParts.All);

                TextRenderer.DrawText(e.Graphics, "Add Card", e.CellStyle.Font, e.CellBounds, e.CellStyle.ForeColor, TextFormatFlags.VerticalCenter | TextFormatFlags.HorizontalCenter);
            ...
            using (SqlConnection sqlConnection = new SqlConnection(DatabaseConnection.ConnectionString))
            {
                ...
                using (SqlCommand schoolIdCommand = new SqlCommand(schoolIdQuery, sqlConnection))
                {
                    ...
                    string query = @"
                        ...
                        LEFT JOIN StudentCard sc ON s.id = sc.studentId
                        WHERE
                        c.schoolId = @schoolId AND sc.studentId IS NULL";
        ...
        private void FilterStudents()
        {
            ...
            string query = @"
                ...
                FROM
                    ...
                    INNER JOIN Parent p ON sp.parentId = p.id
                WHERE
                    c.schoolId = @schoolId AND s.hasOwnCard = 0";
