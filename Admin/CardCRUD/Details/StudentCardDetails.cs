...
{
    public partial class StudentCardDetails : Form
    {
        private string adminUsername;
        private string schoolName;
        private DataGridView _dataGridView;

        public StudentCardDetails(string adminUsername, string schoolName)
        {
            ...
            this.Load += StudentCardDetails_Load;
        ...
        private void _dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == this._dataGridView.Columns["detailsButtonColumn"].Index && e.RowIndex >= 0)
            {
                ...
                StudentCardDetailsInfos studentCardDetailsInfos = new StudentCardDetailsInfos(adminUsername, schoolName, studentId);
                studentCardDetailsInfos.StartPosition = FormStartPosition.Manual;
                studentCardDetailsInfos.Location = this.Location;
                this.Hide();
                studentCardDetailsInfos.ShowDialog();
                this.Close();
            }
        }
        ...
        private void _dataGridView_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex == this._dataGridView.Columns["detailsButtonColumn"].Index)
            {
                ...
                TextRenderer.DrawText(e.Graphics, "Details", e.CellStyle.Font, e.CellBounds, e.CellStyle.ForeColor,
        ...
        private void StudentCardDetails_Load(object sender, EventArgs e)
        {
            LoadStudentData();
        ...
        private void FilterStudents()
        {
            ...
            if (!string.IsNullOrEmpty(nameFilter.Text))
            {
                ...
                parameters.Add(new SqlParameter("@firstName", SqlDbType.NVarChar)
            ...
            if (!string.IsNullOrEmpty(surnameFilter.Text))
            {
                ...
                parameters.Add(new SqlParameter("@lastName", SqlDbType.NVarChar)
                    { Value = "%" + surnameFilter.Text + "%" });
            }

            if (!string.IsNullOrEmpty(numberFilter.Text))
            {
                query += " AND s.studentNumber LIKE @studentNumber";
                parameters.Add(new SqlParameter("@studentNumber", SqlDbType.NVarChar)
                    { Value = "%" + numberFilter.Text + "%" });
            }

            if (!string.IsNullOrEmpty(cardNumberFilter.Text))
            {
                query += " AND sc.cardNumber LIKE @cardNumber";
                parameters.Add(new SqlParameter("@cardNumber", SqlDbType.NVarChar)
                    { Value = "%" + cardNumberFilter.Text + "%" });
            }
